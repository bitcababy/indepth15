...
class Section
  def self.import_from_hash(hash)
    ...
    hash[:semester] = 12 if hash[:semester] == 0
    semester = Durations.old_to_new(hash[:semester])
    ...
    section = Section.new block: block, semester: semester, year: year, \
      days: days, room: room, course: course, teacher: teacher
    if Section.where(_id: section.to_param).exists?
      puts "Section #{section} exists"
      return nil
    end
