...
(($) ->
	$.fn.editable = (target, options) ->
		if "disable" is target
			$(this).data "disabled.editable", true
			return
		if "enable" is target
			$(this).data "disabled.editable", false
			return
		if "destroy" is target
		...
		settings.autowidth = "auto" is settings.width
		settings.autoheight = "auto" is settings.height
		@each ->
			...
			$(this).bind settings.event, (e) ->
				...
				return	if false is onedit.apply(this, [settings, self])
				...
				if 0 is $(self).width()
					...
					settings.width = (if settings.autowidth then $(self).width() else settings.width)	 unless settings.width is "none"
					settings.height = (if settings.autoheight then $(self).height() else settings.height)	 unless settings.height is "none"
				...
				if settings.cssclass
					if "inherit" is settings.cssclass
						form.attr "class", $(self).attr("class")
					else
						form.attr "class", settings.cssclass
				if settings.style
					if "inherit" is settings.style
				...
				else if "submit" is settings.onblur
					input.blur (e) ->
				...
				form.submit (e) ->
					...
					if false isnt onsubmit.apply(form, [settings, self])
						
						# custom inputs call before submit hook. 
						
						# if it returns false abort submitting 
						if false isnt submit.apply(form, [settings, self])
							...
							if $.isFunction(settings.target)
								...
								submitdata["_method"] = "put"	 if "PUT" is settings.method
			...
			@reset = (form) ->
				...
				if @editing
					...
					if false isnt onreset.apply(form, [settings, self])
	...
	$.editable =
		types:
					...
					if settings.submit
						...
						if settings.submit.match(/>$/)
							submit = $(settings.submit).click(->
								form.submit()	 unless submit.attr("type") is "submit"
							)
			...
			text:
				element: (settings, original) ->
					input = $("<input />")
					input.width settings.width	unless settings.width is "none"
					input.height settings.height	unless settings.height is "none"
			...
			textarea:
				element: (settings, original) ->
					...
					else textarea.height settings.height	unless settings.height is "none"
					if settings.cols
						textarea.attr "cols", settings.cols
					else textarea.width settings.width	unless settings.width is "none"
