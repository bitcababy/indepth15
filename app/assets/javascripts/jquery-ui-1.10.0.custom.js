...
(function($, undefined) {
	...
	$.fn.position = function(options) {
		...
		target = $(options.of),
			...
			collision = (options.collision || "flip")
				.split(" "),
			offsets = {};
...
(function($, undefined) {
	...
	$.widget("ui.draggable", $.ui.mouse, {
		...
		_create: function() {
			...
			if (this.options.helper === "original" && !(/^(?:r|a|f)/)
				.test(this.element.css("position"))) {
		...
		_createHelper: function(event) {
			...
			var o = this.options,
				helper = $.isFunction(o.helper) ? $(o.helper.apply(this.element[0], [event])) : (o.helper === "clone" ? this.element.clone()
					.removeAttr("id") : this.element);

			if (!helper.parents("body")
				.length) {
				helper.appendTo((o.appendTo === "parent" ? this.element[0].parentNode : o.appendTo));
			}

			if (helper[0] !== this.element[0] && !(/(fixed|absolute)/)
				.test(helper.css("position"))) {
		...
		_setContainment: function() {
			...
			if (o.containment === "document" || o.containment === "window") {
				...
				o.containment === "document" ? 0 : $(window)
					.scrollTop() - this.offset.relative.top - this.offset.parent.top, (o.containment === "document" ? 0 : $(window)
					.scrollLeft()) + $(o.containment === "document" ? document : window)
					.width() - this.helperProportions.width - this.margins.left, (o.containment === "document" ? 0 : $(window)
					.scrollTop()) + ($(o.containment === "document" ? document : window)
					.height() || document.body.parentNode.scrollHeight) - this.helperProportions.height - this.margins.top];
			}

			if (!(/^(document|window|parent)$/)
				.test(o.containment) && o.containment.constructor !== Array) {
				c = $(o.containment);
				ce = c[0];
				...
				(parseInt($(ce)
					...
					.css("paddingLeft"), 10) || 0), (parseInt($(ce)
					.css("borderTopWidth"), 10) || 0) + (parseInt($(ce)
					.css("paddingTop"), 10) || 0), (over ? Math.max(ce.scrollWidth, ce.offsetWidth) : ce.offsetWidth) - (parseInt($(ce)
					.css("borderLeftWidth"), 10) || 0) - (parseInt($(ce)
					.css("paddingRight"), 10) || 0) - this.helperProportions.width - this.margins.left - this.margins.right, (over ? Math.max(ce.scrollHeight, ce.offsetHeight) : ce.offsetHeight) - (parseInt($(ce)
					.css("borderTopWidth"), 10) || 0) - (parseInt($(ce)
					.css("paddingBottom"), 10) || 0) - this.helperProportions.height - this.margins.top - this.margins.bottom];
...
(function($, undefined) {
	...
	$.ui.intersect = function(draggable, droppable, toleranceMode) {
		...
		x1 = (draggable.positionAbs || draggable.position.absolute)
			.left,
			x2 = x1 + draggable.helperProportions.width,
			y1 = (draggable.positionAbs || draggable.position.absolute)
				.top,
		...
		case "pointer":
			draggableLeft = ((draggable.positionAbs || draggable.position.absolute)
				.left + (draggable.clickOffset || draggable.offset.click)
				.left);
			draggableTop = ((draggable.positionAbs || draggable.position.absolute)
				.top + (draggable.clickOffset || draggable.offset.click)
				.top);
...
(function($, undefined) {
	...
	$.widget("ui.resizable", $.ui.mouse, {
		...
		_create: function() {
			...
			this.handles = o.handles || (!$(".ui-resizable-handle", this.element)
				.length ? "e,s,se" : {
				n: ".ui-resizable-n",
				e: ".ui-resizable-e",
				s: ".ui-resizable-s",
	...
	$.ui.plugin.add("resizable", "containment", {
		...
		stop: function() {
			...
			if (that._helper && !o.animate && (/relative/)
				.test(ce.css("position"))) {
				$(this)
					.css({
			...
			if (that._helper && !o.animate && (/static/)
				.test(ce.css("position"))) {
				$(this)
					.css({
					left: ho.left - cop.left - co.left,
				...
				_alsoResize = function(exp, c) {
					$(exp)
						...
						var el = $(this),
							...
							style = {},
							css = c && c.length ? c : el.parents(ui.originalElement[0])
...
(function($, undefined) {
	...
	$.widget("ui.sortable", $.ui.mouse, {
		...
		_create: function() {
			...
			this.floating = this.items.length ? o.axis === "x" || (/left|right/)
				.test(this.items[0].item.css("float")) || (/inline|table-cell/)
				.test(this.items[0].item.css("display")) : false;
		...
		_setOption: function(key, value) {
			if (key === "disabled") {
				...
				this.widget()
					.toggleClass("ui-sortable-disabled", !! value);
			} else {
		...
		_setContainment: function() {
			...
			if (o.containment === "document" || o.containment === "window") {
				...
				$(o.containment === "document" ? document : window)
					.width() - this.helperProportions.width - this.margins.left, ($(o.containment === "document" ? document : window)
					.height() || document.body.parentNode.scrollHeight) - this.helperProportions.height - this.margins.top];
			}
...
(function($, undefined) {
	...
	$.widget("ui.accordion", {
		...
		_keydown: function(event) { /*jshint maxcomplexity:15*/
		...
		refresh: function() {
			...
			if (options.active === false) {
				...
				if (this.headers.length === this.headers.find(".ui-state-disabled")
					.length) {
					options.active = false;
					this.active = $();
		...
		_refresh: function() {
			...
			if (heightStyle === "fill") {
				...
				this.headers.next()
					.each(function() {
					$(this)
						.height(Math.max(0, maxHeight - $(this)
						.innerHeight() + $(this)
						.height()));
				})
					.css("overflow", "auto");
	...
	$.widget("ui.autocomplete", {
		...
		_create: function() {
			...
			this._on(this.element, {
				keydown: function(event) { /*jshint maxcomplexity:15*/
		...
		_appendTo: function() {
			...
			if (element) {
				element = element.jquery || element.nodeType ? $(element) : this.document.find(element)
					.eq(0);
			}
	...
	$.extend(Datepicker.prototype, { /* Class name added to elements to indicate already configured with a date picker. */
		...
		_attachments: function(input, inst) {
			...
			if (showOn === "button" || showOn === "both") { // pop-up date picker when button clicked
				...
				inst.trigger = $(this._get(inst, "buttonImageOnly") ? $("<img/>")
		...
		_autoSize: function(inst) {
			if (this._get(inst, "autoSize") && !inst.inline) {
				...
				inst.input.attr("size", this._formatDate(inst, date)
					.length);
			}
		},
		...
		_checkOffset: function(inst, offset, isFixed) {
			...
			offset.left -= (isFixed && offset.left === inst.input.offset()
				.left) ? $(document)
				.scrollLeft() : 0;
			offset.top -= (isFixed && offset.top === (inst.input.offset()
				.top + inputHeight)) ? $(document)
				.scrollTop() : 0;
		...
		_checkExternalClick: function(event) {
			...
			if ((($target[0].id !== $.datepicker._mainDivId && $target.parents("#" + $.datepicker._mainDivId)
				.length === 0 && !$target.hasClass($.datepicker.markerClassName) && !$target.closest("." + $.datepicker._triggerClass)
				.length && $.datepicker._datepickerShowing && !($.datepicker._inDialog && $.blockUI))) || ($target.hasClass($.datepicker.markerClassName) && $.datepicker._curInst !== inst)) {
				$.datepicker._hideDatepicker();
			}
		},
		...
		parseDate: function(format, value, settings) {
			...
			iValue = 0,
				...
				shortYearCutoff = (typeof shortYearCutoffTemp !== "string" ? shortYearCutoffTemp : new Date()
					.getFullYear() % 100 + parseInt(shortYearCutoffTemp, 10)),
			...
			if (year === -1) {
				...
				year += new Date()
					...
					.getFullYear() % 100 + (year <= shortYearCutoff ? 0 : -100);
			}
				...
				for (iFormat = 0; iFormat < format.length; iFormat++) {
					if (literal) {
						...
						case "o":
							...
							Math.round((new Date(date.getFullYear(), date.getMonth(), date.getDate())
								.getTime() - new Date(date.getFullYear(), 0, 0)
								.getTime()) / 86400000), 3);
							break;
						case "m":
		...
		_determineDate: function(inst, date, defaultDate) {
			...
			offsetString = function(offset) {
				...
				var date = (offset.toLowerCase()
					.match(/^c/) ? $.datepicker._getDate(inst) : null) || new Date(),
					year = date.getFullYear(),
					month = date.getMonth(),
					day = date.getDate(),
		...
		_getFormatConfig: function(inst) {
			...
			shortYearCutoff = (typeof shortYearCutoff !== "string" ? shortYearCutoff : new Date()
				.getFullYear() % 100 + parseInt(shortYearCutoff, 10));
			return {
...
(function($, undefined) {
	...
	$.widget("ui.dialog", {
		...
		_createWrapper: function() {
			...
			if (!this.element.find("[aria-describedby]")
				.length) {
				this.uiDialog.attr({
		...
		_setOption: function(key, value) { /*jshint maxcomplexity:15*/
...
(function($, undefined) {
	...
	$.widget("ui.menu", {
		...
		_create: function() {
			...
			this._on({
				...
				"click .ui-menu-item:has(a)": function(event) {
					...
					if (!this.mouseHandled && target.not(".ui-state-disabled")
						...
						if (target.has(".ui-menu")
							...
							if (this.active && this.active.parents(".ui-menu")
								.length === 1) {
				...
				focus: function(event, keepActiveItem) {
					...
					var item = this.active || this.element.children(".ui-menu-item")
						.eq(0);
		...
		_keydown: function(event) { /*jshint maxcomplexity:20*/
		...
		_activate: function(event) {
			if (!this.active.is(".ui-state-disabled")) {
				if (this.active.children("a[aria-haspopup='true']")
					.length) {
					this.expand(event);
				} else {
					this.select(event);
		...
		focus: function(event, item) {
			...
			focused = this.active.children("a")
				.addClass("ui-state-focus");
		...
		collapseAll: function(event, all) {
			...
			this.timer = this._delay(function() {
				...
				var currentMenu = all ? this.element : $(event && event.target)
		...
		collapse: function(event) {
			var newItem = this.active && this.active.parent()
...
(function($, undefined) {
	...
	$.widget("ui.slider", $.ui.mouse, {
		...
		_handleEvents: {
			keydown: function(event) { /*jshint maxcomplexity:25*/
...
(function($, undefined) {
	...
	$.widget("ui.tabs", {
		...
		_tabKeydown: function(event) { /*jshint maxcomplexity:15*/
