...
(function() {
		...
		a.tools = {
			...
			convertRgbToHex: function(f) {
				return f.replace(/(?:rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\))/gi, function(g, h, i, j) {
					var k = [h, i, j];
					for (var l = 0; l < 3; l++) k[l] = ('0' + parseInt(k[l], 10)
						.toString(16))
						.slice(-2);
					return '#' + k.join('');
				});
			...
			parseCssText: function(f, g, h) {
				...
				if (!f || f == ';') return i;
				f.replace(/&quot;/g, '"')
					.replace(/\s*([^:;\s]+)\s*:\s*([^;]+)\s*(?=;|$)/g, function(k, l, m) {
					if (g) {
						l = l.toLowerCase();
						if (l == 'font-family') m = m.toLowerCase()
							.replace(/["']/g, '')
							.replace(/\s*,\s*/g, ',');
						m = a.tools.trim(m);
					}
	...
	e.extend(g.prototype, {
		...
		getHead: function() {
			...
			if (!h) h = this.getDocumentElement()
				.append(new d.element('head'), true);
	...
	d.element = function(h, i) {
		if (typeof h == 'string') h = (i ? i.$ : document)
			.createElement(h);
	...
	h.setMarker = function(i, j, k, l) {
		var m = j.getCustomData('list_marker_id') || j.setCustomData('list_marker_id', e.getNextNumber())
			.getCustomData('list_marker_id'),
			n = j.getCustomData('list_marker_names') || j.setCustomData('list_marker_names', {})
	...
	(function() {
		e.extend(h.prototype, {
			...
			append: function(l, m) {
				var n = this;
				if (typeof l == 'string') l = n.getDocument()
					.createElement(l);
			...
			appendBogus: function() {
				...
				if (!l || !l.is || !l.is('br')) {
					var m = b.opera ? n.getDocument()
						.createText('') : n.getDocument()
						.createElement('br');
			...
			contains: c || b.webkit ? function(l) {
				var m = this.$;
				return l.type != 1 ? m.contains(l.getParent()
					.$) : m != l.$ && m.contains(l.$);
			} : function(l) {
			...
			getAttribute: (function() {
				...
				if (c && (b.ie7Compat || b.ie6Compat)) return function(m) {
					...
					case 'contentEditable':
						return q.$.attributes.getNamedItem('contentEditable')
							.specified ? q.$.getAttribute('contentEditable') : null;
					}
					return l.call(q, m);
				};
				else return l;
			...
			isVisible: function() {
				...
				if (l && (b.webkit || b.opera)) {
					m = o.getWindow();
					if (!m.equals(a.document.getWindow()) && (n = m.$.frameElement)) l = new h(n)
						.isVisible();
				}
				return !!l;
			},
			...
			getDocumentPosition: function(l) {
				...
				if (l) {
					...
					if (!D.equals(E) && D.$.frameElement) {
						var F = new h(D.$.frameElement)
							.getDocumentPosition(l);
						m += F.x;
						n += F.y;
					}
				}
			...
			scrollIntoParent: function(l, m, n) {
				...
				function q(C, D) {
					if (/body|html/.test(l.getName())) l.getWindow()
						.$.scrollBy(C, D);
					else {
						l.$.scrollLeft += C;
						l.$.scrollTop += D;
			...
			disableContextMenu: function() {
				this.on('contextmenu', function(l) {
					if (!l.data.getTarget()
						.hasClass('cke_enable_context_menu')) l.data.preventDefault();
				});
			},
			getDirection: function(l) {
				var m = this;
				return l ? m.getComputedStyle('direction') || m.getDirection() || m.getDocument()
					.$.dir || m.getDocument()
					.getBody()
					.getDirection(1) : m.getStyle('direction') || m.getAttribute('dir');
			},
			data: function(l, m) {
				l = 'data-' + l;
			...
			load: function(l, m, n, o) {
				...
				var u = function(y, z) {
					(z ? r : s)
						.push(y);
					if (--q <= 0) {
						o && a.document.getDocumentElement()
							.removeStyle('cursor');
						t(z);
					}
				}, v = function(y, z) {
		...
		a.htmlParser.fragment.prototype = {
			...
			writeHtml: function(q, r) {
				...
				this.filterChildren = function() {
					...
					var u = t.getHtml();
					this.children = new a.htmlParser.fragment.fromHtml(u)
						.children;
					s = 1;
				};
	...
	a.htmlParser.cssStyle = function() {
		...
		(l || '')
			.replace(/&quot;/g, '"')
			.replace(/\s*([^ :;]+)\s*:\s*([^;]+)\s*(?=;|$)/g, function(o, p, q) {
			...
			writeHtml: function(m, n) {
				...
				p.filterChildren = function() {
					if (!u) {
						...
						p.children = new a.htmlParser.fragment.fromHtml(B.getHtml(), 0, p.clone())
							.children;
						u = 1;
					}
				};
	...
	d.comment = function(l, m) {
		if (typeof l == 'string') l = (m ? m.$ : document)
			.createComment(l);
	...
	(function() {
		var l = function(v) {
			...
			if (z.type == 3) {
				...
				z = z.getFirst()
					.insertBeforeMe(v.document.createText(''));
				D = true;
		...
		function o() {
			...
			return function(z) {
				...
				if (z.type == 3 && (z.hasAscendant('pre') || e.trim(z.getText())
					.length)) return false;
		...
		d.range.prototype = {
			...
			createBookmark2: function(v) {
				...
				if (v) {
					if (w.type == 1) {
						B = w.getChild(y);
						if (B && B.type == 3 && y > 0 && B.getPrevious()
							.type == 3) {
							w = B;
							y = 0;
						}
					...
					if (!A) {
						if (x.type == 1) {
							B = x.getChild(z);
							if (B && B.type == 3 && z > 0 && B.getPrevious()
								.type == 3) {
								x = B;
								z = 0;
							}
			...
			insertNode: function(v) {
				...
				if (v.getParent()
					.equals(z.endContainer)) z.endOffset++;
				z.setStartBefore(v);
			},
			...
			splitBlock: function(v) {
				...
				if (v != 'br') {
					if (!A) {
						A = F.fixBlock(true, v);
						B = new d.elementPath(F.endContainer)
							.block;
					}
	...
	(function() {
		...
		function m(n, o, p) {
			...
			if (r.equals(t.getPrevious())) {
				o.startOffset = o.startOffset - r.getLength() - u.getPrevious()
					.getLength();
				r = u.getNext();
		...
		return {
			...
			init: function(n) {
				...
				n.on('menuShow', function(v) {
					var w = v.data[0],
						x = w.element.getElementsByTag('iframe')
							.getItem(0)
							.getFrameDocument();
				...
				if (F && F.length) {
					...
					while (J = I.getNextRange(1)) {
						...
						O.evaluator = function(R) {
							return !!(R.type == 1 && R.getName() in m && !(R.getName() == (E == 1 ? 'p' : 'div') && R.getParent()
								.type == 1 && R.getParent()
								.getName() == 'blockquote') && R.getPosition(P) & 2 && (R.getPosition(Q) & 4 + 16) == 4);
						};
			...
			exec: function(q) {
				...
				if (r == 2) {
					...
					while (K.length > 0) {
						var Q = K.shift();
						J = Q.getParent();
						if (!Q.getPrevious()) Q.remove()
							.insertBefore(J);
						else if (!Q.getNext()) Q.remove()
							.insertAfter(J);
						else {
					...
					if (q.config.enterMode == 2) {
						...
						while (O.length) {
							...
							if (Q.getName() == 'div') {
								...
								var S = R && Q.getPrevious() && !(Q.getPrevious()
									.type == 1 && Q.getPrevious()
									.isBlockBoundary());
								if (S) I.append(q.document.createElement('br'));
								var T = Q.getNext() && !(Q.getNext()
									.type == 1 && Q.getNext()
									.isBlockBoundary());
								while (Q.getFirst()) Q.getFirst()
									.remove()
									.appendTo(I);
	...
	(function() {
		var m = function(y, z) {
			...
			B.on(z, D);
			(b.version > 7 ? A.$ : A.$.selection.createRange())
				.execCommand(z);
		...
		var p = {
			...
			exec: c ? function(y) {
				y.focus();
				if (!y.document.getBody()
					.fire('beforepaste') && !m(y, 'paste')) {
					y.fire('pasteDialog');
					return false;
				}
			} : function(y) {
				try {
					if (!y.document.getBody()
						.fire('beforepaste') && !y.document.$.execCommand('Paste', false, null)) throw 0;
				} catch (z) {
					setTimeout(function() {
						y.fire('pasteDialog');
					}, 0);
					...
					y = e.addFunction(function(E, F) {
						if (E == '?') {
							...
							function H(J) {
								...
								J.name == 'ok' && G(this.getContentElement('picker', 'selectedColor')
									.getValue(), F);
							};
						...
						if (E) {
							...
							I.childRule = F == 'back' ? function(J) {
								...
								return !(J.is('a') || J.getElementsByTag('a')
									.count()) || s(J);
							};
							new a.style(I, {
								color: E
		...
		j.enterkey = {
			enterBlock: function(t, u, v, w) {
				...
				if (y && z) {
					...
					if (B && B.getParent()
						...
						if (!B.getPrevious()
							.getFirst(d.walker.invisible(1))) B.getPrevious()
							.remove();
						if (!B.getNext()
							.getFirst(d.walker.invisible(1))) B.getNext()
							.remove();
				...
				if (!G && !H) {
					if (F.is('li') && (I = F.getFirst(d.walker.invisible(true))) && I.is && I.is('ul', 'ol'))(c ? x.createText('\xa0') : x.createElement('br'))
						.insertBefore(I);
			...
			enterBr: function(t, u, v, w) {
				...
				var x = v.document,
					y = u == 3 ? 'div' : 'p',
					z = v.checkEndOfBlock(),
					A = new d.elementPath(t.getSelection()
						.getStartElement()),
					B = A.block,
					C = B && A.block.getName(),
					D = false;
	...
	(function() {
		...
		function p(v) {
			...
			var w = y.getDialog(),
				x = w.getParentEditor();
			x._.filebrowserSe = y;
			if (!w.getContentElement(y['for'][0], y['for'][1])
				.getInputElement()
				.$.value) return false;
			if (!w.getContentElement(y['for'][0], y['for'][1])
			...
			for (var B in y) {
				...
				if (z.filebrowser.action == 'Browse') {
					...
					if (D) {
						...
						z.filebrowser.url = D;
						z.hidden = false;
						q(v, x.getContents(z['for'][0])
							.get(z['for'][1]), z.filebrowser);
					}
				}
			}
		};
		...
		function u(v, w) {
			...
			if (y) x.getContentElement(y[0], y[1])
				.reset();
		...
		init: function(m) {
			...
			m.ui.addRichCombo('Format', {
				...
				onClick: function(t) {
					...
					m.fire('saveSnapshot');
					var u = q[t],
						v = new d.elementPath(m.getSelection()
							.getStartElement());
	...
	j.add('forms', {
		...
		init: function(m) {
			...
			var o = function(q, r, s) {
				...
				m.ui.addButton(q, {
					label: n.common[q.charAt(0)
						.toLowerCase() + q.slice(1)],
					command: r
				});
				a.dialog.add(r, s);
			}, p = this.path + 'dialogs/';
	...
	(function() {
		...
		function Q(U) {
			return U.replace(/<!--(?!{cke_protected})[\s\S]+?-->/g, function(V) {
				return '<!--' + n + '{C}' + encodeURIComponent(V)
					.replace(/--/g, '%2D%2D') + '-->';
			});
		};
		...
		function T(U, V) {
			...
			U = U.replace(Z, function(ac, ad, ae) {
				return '<!--' + n + (ad ? '{C}' : '') + encodeURIComponent(W[ae])
					.replace(/--/g, '%2D%2D') + '-->';
			});
			...
			exec: function(t) {
				...
				function w(M) {
					var N = C.startContainer,
						O = C.endContainer;
					while (N && !N.getParent()
						.equals(M)) N = N.getParent();
					while (O && !O.getParent()
						.equals(M)) O = O.getParent();
					if (!N || !O) return;
					var P = N,
						Q = [],
						R = false;
			...
			exec: function(N) {
				...
				while ((T = Y.getNextRange()) && ++Z) {
					...
					while (ae = ad.getNextParagraph()) {
						...
						if (aj) continue;
						var ao = ak;
						if (ao.getCustomData('list_group_object_' + Z)) ao.getCustomData('list_group_object_' + Z)
							.contents.push(ae);
						else {
							an = {
								root: ao,
	...
	(function() {
		...
		function r(s) {
			...
			var t = h.createFromHtml('<iframe frameborder="0" tabindex="-1" src="javascript:void((function(){document.open();' + (b.isCustomDomain() ? "document.domain='" + this.getDocument()
				.$.domain + "';" : '') + 'document.close();' + '})())"' + ' style="display:block;position:absolute;z-index:-1;' + 'progid:DXImageTransform.Microsoft.Alpha(opacity=0);' + '"></iframe>');
				...
				s.addCommand('maximize', {
					...
					exec: function() {
						...
						if (this.state == 2) {
							...
							u.getDocumentElement()
								.setStyles(H);
							!b.gecko && u.getDocumentElement()
								.setStyle('position', 'fixed');
							!(b.gecko && b.quirks) && u.getBody()
								.setStyles(H);
	...
	j.add('pagebreak', {
		init: function(m) {
			...
			var n = ['{', 'background: url(' + a.getUrl(this.path + 'images/pagebreak.gif') + ') no-repeat center center;', 'clear: both;', 'width:100%; _width:99.9%;', 'border-top: #999999 1px dotted;', 'border-bottom: #999999 1px dotted;', 'padding:0;', 'height: 5px;', 'cursor: default;', '}'].join('')
				.replace(/;/g, ' !important;');
			m.addCss('div.cke_pagebreak' + n);
			b.opera && m.on('contentDom', function() {
				m.document.on('click', function(o) {
					var p = o.data.getTarget();
					if (p.is('div') && p.hasClass('cke_pagebreak')) m.getSelection()
						.selectElement(p);
				});
			});
		},
		...
		var m, n = {
			...
			exec: function(p) {
				var q, r = p.config,
					...
					t = b.isCustomDomain();
				if (r.fullPage) q = p.getData()
					.replace(/<head>/, '$&' + s)
					.replace(/[^>]*(?=<\/title>)/, '$& &mdash; ' + p.lang.preview);
				else {
					var u = '<body ',
		...
		var p = function() {
			var t = this,
				u = function() {
					var y = t.config,
						z = {};
					z.srcNodeRef = t.document.getWindow()
						.$.frameElement;
		...
		j.scayt = {
			...
			loadEngine: function(t) {
				...
				if (window.scayt == undefined) {
					...
					a.document.getHead()
						.append(a.document.createElement('script', {
						attributes: {
							type: 'text/javascript',
							async: 'true',
							src: w
			...
			init: function(t) {
				...
				t.ui.add('Scayt', 'menubutton', {
					...
					onMenu: function() {
						...
						t.getMenuItem('scaytToggle')
							.label = C[E ? 'disable' : 'enable'];
						var F = q.getUiTabs(t);
						return {
							scaytToggle: 2,
	...
	(function() {
		...
		j.add('showblocks', {
			...
			init: function(r) {
				...
				r.addCss(m.replace(n, 'background-image: url(' + a.getUrl(this.path) + 'images/block_')
					.replace(o, 'cke_show_blocks ')
					.replace(p, r.lang.dir == 'rtl' ? 'right' : 'left'));
	...
	(function() {
		var m = 'cke_show_border',
			n, o = (b.ie6Compat ? ['.%1 table.%2,', '.%1 table.%2 td, .%1 table.%2 th', '{', 'border : #d3d3d3 1px dotted', '}'] : ['.%1 table.%2,', '.%1 table.%2 > tr > td, .%1 table.%2 > tr > th,', '.%1 table.%2 > tbody > tr > td, .%1 table.%2 > tbody > tr > th,', '.%1 table.%2 > thead > tr > td, .%1 table.%2 > thead > tr > th,', '.%1 table.%2 > tfoot > tr > td, .%1 table.%2 > tfoot > tr > th', '{', 'border : #d3d3d3 1px dotted', '}'])
				.join('');
		n = o.replace(/%2/g, m)
		...
		j.add('showborders', {
			...
			init: function(q) {
				...
				q.on('removeFormatCleanup', function(s) {
					var t = s.data;
					if (q.getCommand('showborders')
						.state == 1 && t.is('table') && (!t.hasAttribute('border') || parseInt(t.getAttribute('border'), 10) <= 0)) t.addClass(m);
				});
			},
				...
				for (var Q = M; Q <= O; Q++) {
					...
					for (var T = 0; T < R.length; T++) {
						...
						if (U.$.rowSpan == 1) U.remove();
							...
							if (V == Q) {
								var W = K[Q + 1];
								W[T - 1] ? U.insertAfter(new h(W[T - 1])) : new h(J.$.rows[Q + 1])
									.append(U, 1);
							}
						}
						T += U.$.colSpan - 1;
						...
						if (u = t.getAscendant('td', true) || t.getAscendant('th', true)) {
							...
							if (!(w || q)) {
								...
								for (var A = 0, B = y.length; A < B; A++) {
									var C = z.append(new h(y[A], r.document)
										.clone(false, false));
									!c && C.appendBogus();
								}
		...
		o.prototype = {
			...
			restoreImage: function(s) {
				...
				if (s.bookmarks) u.selectBookmarks(s.bookmarks);
					...
					var v = w.editor.document.getBody()
						.$.createTextRange();
					v.collapse(true);
					v.select();
				}
	...
	(function() {
		...
		function s(E) {
			...
			if (b.opera) {
				...
				if (H.block) {
					var I = a.htmlParser.fragment.fromHtml(E, false)
						.children;
		...
		function t(E) {
			var F = this.getSelection(),
				G = F.getStartElement()
					.hasAscendant('pre', true) ? 2 : this.config.enterMode,
				H = G == 2,
			...
			if (b.gecko || b.webkit) {
				...
				for (var M = 0; M < K.elements.length; M++) {
					...
					if (N in f.$inline) L.unshift(K.elements[M].getOuterHtml()
						.match(/^<.*?>/));
		...
		function z(E) {
			return E.type == 3 && e.trim(E.getText())
				.match(/^(?:&nbsp;|\xa0)$/);
		};
		...
		function C(E) {
			...
			if (!J || J == 1 && I.type == 1 && I.hasAttribute('_moz_editor_bogus_node')) {
				...
				G.$.dispatchEvent(O);
				if (M != L.$.scrollTop || N != L.$.scrollLeft) K.getWindow()
					.$.scrollTo(N, M);
				J && H.getFirst()
					.remove();
				G.getBody()
		...
		j.add('wysiwygarea', {
			...
			init: function(E) {
				...
				E.on('editingBlockReady', function() {
					...
					function W(X) {
						...
						if (!(c || b.opera)) Y.on('mousedown', function(ag) {
							var ah = ag.data.getTarget();
							if (ah.is('img', 'hr', 'input', 'textarea', 'select')) E.getSelection()
								.selectElement(ah);
						});
						...
						Y.on('click', function(ag) {
							ag = ag.data;
							if (ag.getTarget()
								.is('a') && ag.$.button != 2) ag.preventDefault();
						});
						if (b.webkit) {
							...
							Y.on('click', function(ag) {
								if (ag.data.getTarget()
									.is('input', 'select')) ag.data.preventDefault();
							});
							Y.on('mouseup', function(ag) {
								if (ag.data.getTarget()
									.is('input', 'textarea')) ag.data.preventDefault();
							});
						}
						var ac = c ? N : X;
						ac.on('blur', function() {
						...
						ae.attach(Y);
						Y.getDocumentElement()
							.addClass(Y.$.compatMode);
						...
						if (c && E.config.enterMode != 1) Y.on('selectionchange', function() {
							...
							if (ai && ag.getHtml()
								.match(/^<p>&nbsp;<\/p>$/i) && ai.startContainer.equals(ag)) setTimeout(function() {
								ai = E.getSelection()
									.getRanges()[0];
						...
						setTimeout(function() {
							...
							if (E.config.disableObjectResizing) try {
								...
								E.document.getBody()
									.on(c ? 'resizestart' : 'resize', function(aj) {
					...
					E.addMode('wysiwyg', {
						...
						getData: function() {
							var X = E.config,
								...
								ac = Y ? ab.getDocumentElement()
									.getOuterHtml() : ab.getBody()
									.getHtml();
				...
				E.on('contentDom', function() {
					var M = E.document.getElementsByTag('title')
						.getItem(0);
				...
				if (R.config.dialog_startupFocusTab && ah._.pageCount > 1) {
					...
					if (av._.editor.mode == 'wysiwyg' && c) {
						var as = R.document.$.selection,
							at = as.createRange();
						if (at) if (at.parentElement && at.parentElement()
							.ownerDocument == R.document.$ || at.item && at.item(0)
							.ownerDocument == R.document.$) {
							var au = document.body.createTextRange();
							au.moveToElementText(av.getElement()
								.getFirst()
								.$);
							au.collapse(true);
							au.select();
		...
		a.dialog.prototype = {
			...
			resize: (function() {
				return function(R, S) {
					...
					if (T._.editor.lang.dir == 'rtl' && T._.position) T._.position.x = a.document.getWindow()
						.getViewPaneSize()
						.width - T._.contentSize.width - parseInt(T._.element.getFirst()
						.getStyle('right'), 10);
					T._.contentSize = {
						width: R,
						height: S
			...
			show: function() {
				var R = this._.element,
					S = this.definition;
				if (!(R.getParent() && R.getParent()
					.equals(a.document.getBody()))) R.appendTo(a.document.getBody());
				...
				this._.element.getFirst()
					.setStyle('z-index', a.dialog._.currentZIndex += 10);
				if (a.dialog._.currentTop === null) {
					...
					var U = this._.parentDialog.getElement()
						.getFirst();
			...
			commitContent: function() {
				var R = arguments;
				this.foreach(function(S) {
					if (c && this._.currentFocusIndex == S.focusIndex) S.getInputElement()
						.$.blur();
			...
			hide: function() {
				...
				if (!this._.parentDialog) I();
					...
					var S = this._.parentDialog.getElement()
						.getFirst();
			...
			getValueOf: function(R, S) {
				return this.getContentElement(R, S)
					.getValue();
			},
			...
			var U = R.getParentEditor(),
				V, W, X, Y, Z, aa, ab = e.addFunction(function(ae) {
					...
					var af = R.parts.contents,
						ag = af.$.getElementsByTagName('iframe')
							.length;
					if (ag) {
			...
			k.dialog = {
				uiElement: function(R, S, T, U, V, W, X) {
					...
					var Y = (U.call ? U(S) : U) || 'div',
						Z = ['<', Y, ' '],
						aa = (V && V.call ? V(S) : V) || {}, ab = (W && W.call ? W(S) : W) || {}, ac = (X && X.call ? X.call(this, R, S) : X) || '',
				...
				vbox: function(R, S, T, U, V) {
					...
					var W = this._.children = S,
						...
						Z = function() {
							...
							aa.push('"');
							aa.push('align="', e.htmlEncode(V && V.align || (R.getParentEditor()
								.lang.dir == 'ltr' ? 'left' : 'right')), '" ');
							aa.push('><tbody>');
	...
	(function() {
		...
		a.style.prototype = {
			...
			applyToRange: function(T) {
				var U = this;
				return (U.applyToRange = U.type == 2 ? t : U.type == 1 ? x : U.type == 3 ? v : null)
					.call(U, T);
			},
			removeFromRange: function(T) {
				var U = this;
				return (U.removeFromRange = U.type == 2 ? u : U.type == 1 ? y : U.type == 3 ? w : null)
					.call(U, T);
			},
		...
		function Q(T) {
			var U = {};
			T.replace(/&quot;/g, '"')
				.replace(/\s*([^ :;]+)\s*:\s*([^;]+)\s*(?=;|$)/g, function(V, W, X) {
				U[W] = X;
			});
			return U;
		};
	...
	a.editor.prototype.getStylesSet = function(m) {
		if (!this._.stylesDefinitions) {
			...
			a.stylesSet.addExternal(q, r ? p.slice(1)
				.join(':') : s + 'styles/' + q + '.js', '');
			...
			getNextParagraph: function(s) {
				...
				if (!S._.started) {
					...
					if (!u.collapsed) {
						...
						if (S._.lastNode && S._.lastNode.type == 3 && !e.trim(S._.lastNode.getText()) && S._.lastNode.getParent()
							.isBlockBoundary()) {
				...
				if (!t) {
					...
					} else if (!v) S._.nextNode = t.equals(B) ? null : r(u.getBoundaryNodes()
						.endNode, 1, B);
				}
				if (x) {
					var Q = t.getPrevious();
					if (Q && Q.type == 1) if (Q.getName() == 'br') Q.remove();
					else if (Q.getLast() && Q.getLast()
				...
				proto: {
					createPanel: function(n) {
						...
						var p = this._.panelDefinition || {}, q = this._.panelDefinition.block,
							...
							u = this;
						s.onShow = function() {
							if (u.className) this.element.getFirst()
								.addClass(u.className + '_panel');
							u.setState(1);
							o.on = 1;
						...
						s.onHide = function(v) {
							if (u.className) this.element.getFirst()
								.removeClass(u.className + '_panel');
			...
			proto: {
					...
					e.setTimeout(function() {
						var E = e.bind(function() {
							...
							if (v.autoSize) {
								...
								u._.currentBlock.element.setStyle('display', 'none')
									.removeStyle('display');
							...
							if (c) {
								var T = new h(w.$.offsetParent),
									U = T;
								if (U.getName() == 'html') U = U.getDocument()
									.getBody();
								if (U.getComputedStyle('direction') == 'rtl') if (b.ie8Compat) B -= w.getDocument()
									.getDocumentElement()
									.$.scrollLeft * 2;
				...
				hide: function(p) {
					...
					if (r.visible && (!r.onHide || r.onHide.call(r) !== true)) {
						r.hideChild();
						b.gecko && r._.iframe.getFrameDocument()
							.$.activeElement.blur();
						...
						if (q) {
							if (b.webkit && q.type) q.getWindow()
								.$.focus();
							q.focus();
						}
					}
		...
		a.menu = e.createClass({
			...
			_: {
				onShow: function() {
					...
					for (var r = 0; r < p.length; r++) {
						...
						if (s) for (var t in s) {
							...
							if (u && (!u.command || v.editor.getCommand(u.command)
								.state)) {
								u.state = s[t];
								v.add(u);
							}
				...
				showSubMenu: function(n) {
					...
					var u = v._.panel.getBlock(v.id)
						.element.getDocument()
						.getById(v.id + String(n));
					o.show(u, 2);
				}
			},
			proto: {
		...
		a.menuItem = e.createClass({
			...
			proto: {
				render: function(n, o, p) {
					...
					var u = w.getItems;
					p.push('<span class="cke_menuitem' + (w.icon && w.icon.indexOf('.png') == -1 ? ' cke_noalphafix' : '') + '">' + '<a id="', q, '" class="', s, '" href="javascript:void(\'', (w.label || '')
						.replace("'", ''), '\')" title="', w.label, '" tabindex="-1"_cke_focus=1 hidefocus="true" role="menuitem"' + (u ? 'aria-haspopup="true"' : '') + (r == 0 ? 'aria-disabled="true"' : '') + (r == 1 ? 'aria-pressed="true"' : ''));
		...
		j.add('editingblock', {
			init: function(n) {
				...
				n.on('getSnapshot', function(o) {
					if (n.mode) o.data = n.getMode()
						.getSnapshotData();
				});
				n.on('loadSnapshot', function(o) {
					if (n.mode) n.getMode()
						.loadSnapshotData(o.data);
				});
	...
	(function() {
				...
				if (b.webkit) {
					...
					function H() {
						...
						if (J) {
							J.setText(E);
							if (F) {
								I.$.defaultView.getSelection()
									.setPosition(J.$, J.getLength());
								F = 0;
							}
						}
					};
						...
						if (I.$.compatMode != 'BackCompat') {
							if (b.ie7Compat || b.ie6Compat) {
								...
								L.on('mousedown', function(V) {
									...
									V = V.data;
									if (V.getTarget()
										.is('html') && V.$.x < L.$.clientWidth && V.$.y < L.$.clientHeight) {
							...
							if (b.ie8) {
								...
								function S() {
									...
									var V = a.document.$.selection,
										W = V.createRange();
									if (V.type != 'None' && W.parentElement()
										.ownerDocument == I.$) W.select();
								};
							}
						}
		...
		d.selection = function(D) {
			...
			if (c) try {
				var F = G.getNative()
					.createRange();
				if (!F || F.item && F.item(0)
					.ownerDocument != G.document.$ || F.parentElement && F.parentElement()
					.ownerDocument != G.document.$) throw 0;
			} catch (H) {
				G.isInvalid = true;
			}
			return G;
		...
		d.selection.prototype = {
			getNative: c ? function() {
				...
				return this._.cache.nativeSel || (this._.cache.nativeSel = this.document.getWindow()
					.$.getSelection());
			},
			getType: c ? function() {
				var D = this._.cache;
					...
					F = e.tryThese(function() {
						...
						if (G) {
							...
							var N = e.extend(K, f.$listItem, f.$tableContent),
								...
								P = function(Q, R) {
									return function(S, T) {
										if (S.type == 3 && (!e.trim(S.getText()) || S.getParent()
											.data('cke-bookmark'))) return true;
										var U;
										if (S.type == 1) {
											U = S.getName();
											if (U == 'br' && R && S.equals(S.getParent()
												.getBogus())) return true;
			...
			getSelectedText: function() {
				...
				var E = '',
					F = this.getNative();
				if (this.getType() == 2) E = c ? F.createRange()
					.text : F.toString();
		...
		d.range.prototype.select = c ? function(D) {
			...
			if (L) {
				...
				var N = K.getNext(z);
				F = !(N && N.getText && N.getText()
					.match(B)) && (D || !K.hasPrevious() || K.getPrevious()
					.is && K.getPrevious()
					.is('br'));
				G = O.document.createElement('span');
				G.setHtml('&#65279;');
				G.insertBefore(K);
				if (F) O.document.createText('\ufeff')
					.insertBefore(K);
			}
			O.setStartBefore(K);
			K.remove();
		...
		proto: {
			...
			createPanel: function(m) {
				...
				var n = this._.panelDefinition,
					...
					s = this;
				q.onShow = function() {
					if (s.className) this.element.getFirst()
						.addClass(s.className + '_panel');
					s.setState(1);
				...
				q.onHide = function(t) {
					if (s.className) this.element.getFirst()
						.removeClass(s.className + '_panel');
	...
	(function() {
		...
		e.extend(k.dialog, {
			...
			select: function(u, v, w) {
				...
				var y = function() {
					...
					t(z);
					for (var D = 0, E; D < v.items.length && (E = v.items[D]); D++) B.push('<option value="', e.htmlEncode(E[1] !== undefined ? E[1] : E[0])
						.replace(/"/g, '&quot;'), '" /> ', e.htmlEncode(E[0]));
					if (typeof z.inputStyle != 'undefined') z.style = z.inputStyle;
			...
			fileButton: function(u, v, w) {
				...
				z.onClick = function(B) {
					...
					if (!A || A.call(this, B) !== false) {
						u.getContentElement(C[0], C[1])
							.submit();
						this.disable();
					}
				};
				u.on('load', function() {
					u.getContentElement(v['for'][0], v['for'][1])
						._.buttons.push(y);
				});
		...
		k.dialog.labeledElement.prototype = e.extend(new k.dialog.uiElement(), {
			setLabel: function(u) {
				...
				if (v.getChildCount() < 1) new d.text(u, a.document)
					.appendTo(v);
				else v.getChild(0)
					.$.nodeValue = u;
				return this;
		...
		k.dialog.select.prototype = e.extend(new k.dialog.labeledElement(), {
			...
			add: function(u, v, w) {
				var x = new h('option', this.getDialog()
					.getParentEditor()
					.document),
		...
		k.dialog.radio.prototype = e.extend(new k.dialog.uiElement(), {
			setValue: function(u, v) {
				var w = this._.children,
					x;
				for (var y = 0; y < w.length && (x = w[y]); y++) x.getElement()
					.$.checked = x.getValue() == u;
				!v && this.fire('change', {
					value: u
				});
			},
		...
		k.dialog.file.prototype = e.extend(new k.dialog.labeledElement(), q, {
			getInputElement: function() {
				var u = a.document.getById(this._.frameId)
					.getFrameDocument();
	...
	k.panel.block = e.createClass({
		$: function(m, n) {
			var o = this;
			o.element = m.append(m.getDocument()
				.createElement('div', {
				attributes: {
					tabIndex: -1,
		...
		_: {
			markItem: function(m) {
				...
				if (b.webkit || b.opera) o.getDocument()
					.getWindow()
					.focus();
				o.focus();
				p.onMark && p.onMark(o);
		...
		proto: {
			...
			onKeyDown: function(m) {
				...
				case 'mouseup':
					o = r._.focusIndex;
					q = o >= 0 && r.element.getElementsByTag('a')
						.getItem(o);
					if (q) q.$[n] ? q.$[n]() : q.$['on' + n]();
					return false;
				}
		...
		onLoad: function() {
			...
			k.listBlock = e.createClass({
				...
				proto: {
					...
					hideGroup: function(m) {
						var n = this.element.getDocument()
							.getById(this._.groups[m]),
							o = n && n.getNext();
						if (n) {
	...
	a.themes.add('default', (function() {
		...
		return {
			build: function(o, p) {
				...
				D.getChild([1, 0, 0, 0, 0])
					.unselectable();
				D.getChild([1, 0, 0, 0, 2])
					.unselectable();
				...
				o.on('contentDirChanged', function(E) {
					...
					G && G.getParent()
						.getParent()[F]('cke_mixed_dir_content');
				});
	...
	a.editor.prototype.resize = function(m, n, o, p) {
		...
		var q = v.container,
			...
			s = b.webkit && v.document && v.document.getWindow()
				.$.frameElement,
			t = p ? q.getChild(1) : q;
